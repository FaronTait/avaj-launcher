    ...
    public void updateConditions() 
    {
        ...
        switch (weather) {
            case "RAIN":
                ...
                writr.writetofile("JetPlane#" + this.name + "(" + this.id + "): Don't you just hate it when the Gods start crying.");
                break;
            case "SNOW":
                this.coordinates = new Coordinates(copyLong, copyLat, copyHeight - 7);
                writr.writetofile("JetPlane#" + this.name + "(" + this.id + "): Perfect movie weather.");
                break;
            case "FOG":
                this.coordinates = new Coordinates(copyLong, copyLat + 1, copyHeight);
                writr.writetofile("JetPlane#" + this.name + "(" + this.id + "): Is it just me or can no one else see through this??");
                break;
            case "SUN":
                this.coordinates = new Coordinates(copyLong, copyLat + 10, copyHeight + 2);
                writr.writetofile("JetPlane#" + this.name + "(" + this.id + "): My mom always told me the sun is bad for you.");
                break;            
            default:
